...
namespace WebdriverFramework.Framework.util
{
    ...
    public class OpenPopUtils
    {
        ...
        public static HashSet<Message> FetchUnseenMessages(string hostname, int port, bool useSsl, string username, string password, List<string> seenUids)
        {
            // The client disconnects from the server when being disposed
            using (Pop3Client client = new Pop3Client())
            {
                ...
                HashSet<Message> newMessages = new HashSet<Message>();
